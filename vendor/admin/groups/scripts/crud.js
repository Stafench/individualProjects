...
let row = $(`
<tr>
    <th id="name">Наименование группы</th>
...
function addRowsInTable(nameColumn = 'id_group', orderBy = 'ASC'){
  $.ajax({
    ...
    success: function(response) {
      ...
      sortNowNameColumn = nameColumn;
        response.forEach(row => {
            ...
            row = $(`
        <tr class="rowTable" data-id="${row.id_group}">
...
$('#link-back').on('click', function(event) {
  ...
  $('#mainTableH #default').click(function (e) { 
    e.preventDefault();
    if(sortNowNameColumn === 'id_group' && sortNowOrderBy === 'ASC'){
